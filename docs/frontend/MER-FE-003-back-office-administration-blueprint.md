...
| Frontend decision | The import form may accept a UTF-8 CSV as a browser-side input convenience. Internal Web validates whether each CSV row can become the existing structured import shape, presents a bounded review, and sends only the mapped JSON rows; Partner remains authoritative for import validation and reconciliation. |
...
Create, edit, status, and import controls appear only when the actor also has exact `partner:manage`. Raw CSV content, parsed employee source data, and the mapped command remain in mounted page memory only; the browser does not persist or upload the source file. Consequential commands do not retry automatically. An unknown import result retains the exact `requestId` and unchanged mapped payload in mounted page memory only and offers explicit exact replay. Confirmed import success refreshes employee and import-history reads. The browser does not label a batch authoritative, duplicate Partner validation rules, or refresh Customer employment links.

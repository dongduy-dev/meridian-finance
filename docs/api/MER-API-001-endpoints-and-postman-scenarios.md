...
OCR state is advisory. `PENDING`, `PROCESSING`, `COMPLETED`, or `FAILED` does not create or modify Customer or Loan state, establish consent or verification, accept or waive evidence, or block manual intake. Staff OCR result review and application of reviewed suggestions are separate contracts.

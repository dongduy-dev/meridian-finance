...
CREATE TABLE refresh_token_sessions (
    ...
    CONSTRAINT fk_refresh_token_sessions_user
        FOREIGN KEY (user_id)
        REFERENCES users (id),
    CONSTRAINT uq_refresh_token_sessions_token_digest UNIQUE (token_digest),
...
-- Partner eligibility manual-review final state (introduced by V63).
...
-- V39 UCL manual verification and review safety
...
-- V40 UCL pricing and approved-offer support
...
-- V41 UCL operational-contract repayment-method support
...
-- V42 product-aware repayment, settlement, and closure reconciliation

-- Make the current repayment, outcome, and closure reconciliation
-- product-aware without introducing a second servicing model.
...
BEGIN
    ...
    IF EXISTS (
        ...
        WHERE application_row.product_code NOT IN (
           ...
           OR (
                application_row.product_code = 'UNSECURED_CONSUMER_LOAN'
                AND EXISTS (
                    SELECT 1
                    FROM salary_advance_limit_movements release
...
-- V45 Collateral Loan manual verification and correction safety
ALTER TABLE loan_correction_tasks
    DROP CONSTRAINT chk_loan_correction_tasks_document_type,
    ADD CONSTRAINT chk_loan_correction_tasks_document_type CHECK (
        document_type IN (
...
-- V46 Collateral Loan pricing and approved-offer support
...
-- V53 add Partner administration command persistence and audit vocabulary
...
-- V54 add Loan Product administration audit vocabulary
ALTER TABLE audit_events
    DROP CONSTRAINT chk_audit_events_entity_type,
    DROP CONSTRAINT chk_audit_events_action,
    ADD CONSTRAINT chk_audit_events_entity_type CHECK (entity_type IN (
...
-- V55 add Internal User administration and authorization freshness
ALTER TABLE users
    ADD COLUMN authorization_version BIGINT NOT NULL DEFAULT 0,
    ADD CONSTRAINT chk_users_authorization_version_non_negative
...
-- V56 Staff-assisted origination foundation
...
-- V56 audit vocabulary final state
ALTER TABLE audit_events
    DROP CONSTRAINT chk_audit_events_entity_type,
    DROP CONSTRAINT chk_audit_events_action,
    ADD CONSTRAINT chk_audit_events_entity_type CHECK (entity_type IN (
...
-- V59 Staff OCR review and correction
...
-- V60: evidenced Staff-assisted downstream Customer decisions.
...
-- V62: Staff-assisted UCL Customer cancellation.

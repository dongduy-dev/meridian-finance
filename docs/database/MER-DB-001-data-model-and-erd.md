...
> **Model authority and state:** Sections 1-13 are a high-level logical/current-plus-target model; names in the ERD are not an exact physical-schema inventory. Executable Flyway migrations are authoritative for deployed structure, and `MER-DB-CURRENT-SCHEMA.sql` is the current human-readable V1-V47 snapshot. The current physical model has no `refresh_tokens` or OCR tables. It includes one or more Loan-owned `collaterals` fact rows per application at the database level, sequenced immutable application-owned `collateral_loan_verifications` cycles with source-correction and restricted completion evidence, Document-owned `COLLATERAL_OWNERSHIP_EVIDENCE` checklist evidence, executable Collateral default pricing and exact allowed terms, Collateral approved-offer and operational-contract snapshots, and final Collateral monthly schedules created during manual-disbursement activation. It also includes sequenced application-owned `unsecured_consumer_loan_verifications` rows for immutable UCL product-verification cycles and authoritative manual-review evidence, UCL-specific document categories, executable UCL default pricing and terms, immutable UCL approved-offer and operational-contract snapshots, and final UCL monthly schedules created during manual-disbursement activation. The platform uses `manual_disbursements` rather than the conceptual `disbursement_records`, and servicing uses `repayment_schedule_items`, typed `repayment_transactions`, `repayment_allocations`, `repayment_installment_progress`, `repayment_operation_outcomes`, immutable approved-settlement and closure evidence, LoanAccount/installment status-transition tables, and product-aware exposure reconciliation rather than a single `repayment_records` table. Salary Advance, UCL, and Collateral all use the common servicing structures; Collateral requires zero Salary Advance exposure evidence.
...
- `loan_product_policies` - configurable product policy values such as amount ranges, allowed terms, pricing method and rate, fee, repayment method, offer validity, and product-specific validation settings. The active UCL `DEFAULT_POLICY` stores `FLAT_ORIGINAL_PRINCIPAL`, `0.018000`, zero fee, `MONTHLY_INSTALLMENT`, and seven-day validity. The active Collateral `DEFAULT_POLICY` uses the same method, fee, repayment, and validity fields with rate `0.015000` and exact terms 6, 12, 18, and 24 months.
...
- `approved_offers` - immutable customer-facing approved-offer snapshots generated after approval and before customer acceptance. Repayment method permits Salary Advance `ON_SALARY_DATE` and the executable UCL and Collateral `MONTHLY_INSTALLMENT` values.
...
- `loan_contracts` - immutable versioned operational contract snapshot, purpose-protected destination, command identities, and controlled lifecycle evidence. Executable repayment methods are Salary Advance `ON_SALARY_DATE` and UCL and Collateral `MONTHLY_INSTALLMENT`.
...
`collaterals` contains the structured facts owned by Loan: type, description, Customer-estimated value, Customer-submitted ownership status, condition note, and creation time. The estimate supports manual assessment only; no loan-to-value is persisted or calculated. The public correction workflow does not version or modify these submitted facts.

`collateral_loan_verifications` follows the established numbered-cycle model. The `(loan_application_id, verification_sequence)` tuple is unique; sequence 1 has no source correction, and each later cycle has one unique `source_correction_request_id` for the same application. Pending rows carry no completion evidence. Terminal `VERIFIED`, `FAILED`, and `REQUIRES_MORE_INFORMATION` rows require reviewer, review time, and a nonblank assessment note; Loan trims the note before persistence. Review time cannot precede creation. A pending row may complete exactly once; completed rows and all cycle identity fields are immutable. A deferred source-reconciliation trigger requires the immediately preceding cycle to be completed and the source correction to be resubmitted before the later cycle's creation time.
...
`loan_applications` keeps `product_code` and `product_type` snapshots for reporting and historical stability. Product-specific request details can start in `product_details` JSONB when they are simple; data with lifecycle rules, review notes, or reporting needs should graduate into dedicated tables. UCL uses the active `DEFAULT_POLICY` and its exact 3, 6, 9, and 12-month term rows to create immutable offer terms and one provisional repayment item per month. Collateral uses its exact 6, 12, 18, and 24-month policy rows to create the same common offer shape at its approved 1.5% monthly flat rate without Salary Advance exposure evidence. UCL and Collateral operational contracts copy accepted terms and items exactly, while manual disbursement creates a common LoanAccount and authoritative final monthly schedule. Both products use common servicing with zero Salary Advance exposure evidence.
...
- One Collateral Loan `loan_applications` record has one or more `collaterals` rows at the physical-model level; the public origination endpoint creates exactly one, and verification fails safely if that API invariant is violated. It has one or more numbered `collateral_loan_verifications` rows; the latest cycle is authoritative and earlier completed cycles remain immutable evidence.
...
- One accepted Salary Advance, UCL, or Collateral application may produce versioned `loan_contracts`; each version copies the accepted offer terms and repayment items and captures one purpose-protected disbursement destination.
...
The V29, V31, V37, V43, V44, V45, V46, and V47 migrations preflight the prior state they replace or extend. They reject missing, repeated, or incompatible constraint state before mutation. `MER-DB-CURRENT-SCHEMA.sql` reflects the stable V1-V47 physical result, including Collateral origination, sequenced manual-verification/correction structures, executable Collateral policy/terms and common approved-offer support, Collateral-aware common servicing reconciliation, sequenced UCL verification and source-correction evidence, executable UCL offer and contract repayment-method support, and product-aware returned-correction cancellation; migration preflight machinery is intentionally kept only in executable Flyway history.
...
## 20. Collateral Loan pricing and approved offers (V46)

V46 activates the existing Collateral `DEFAULT_POLICY` as an executable exact-request offer policy. It sets `FLAT_ORIGINAL_PRINCIPAL`, monthly rate `0.015000`, zero fee, `MONTHLY_INSTALLMENT`, and seven-calendar-day validity, and reconciles the policy-term rows to exactly 6, 12, 18, and 24 months. The migration uses the existing product, policy, offer, and provisional-item tables; it adds no Collateral pricing table, loan-to-value column, counteroffer structure, contract structure, servicing structure, or Salary Advance exposure artifact.

The common `approved_offers` and `approved_offer_repayment_items` constraints continue to enforce one offer per application, one unique installment number per offer, positive whole-VND principal and totals, nonnegative whole-VND interest and fees, arithmetic equality, and valid offer-state timestamps. V46 deliberately does not add a common ApprovedOffer update trigger or redesign accepted shared persistence. Application transactions remain responsible for persisting the decision, offer, items, status history, and audit as one business outcome; PostgreSQL uniqueness remains the final duplicate-offer guard.

## 21. Collateral Loan servicing reconciliation (V47)

V47 replaces only the three common deferred reconciliation functions for repayment servicing, repayment operation outcomes, and LoanAccount closure. It adds explicit Collateral branches while preserving the Salary Advance and UCL branches and the existing trigger bindings. Collateral ordinary repayment, contractual payoff, Administrative Full-Balance Settlement, overdue evaluation and cure, and administrative closure reuse the common payment, allocation, progress, outcome, history, settlement, closure, and audit tables; no Collateral-specific servicing table is added.

Collateral outcome and closure reconciliation require `principal_released = 0` and no `DISBURSED_TO_USED` or `REPAID_RELEASED` Salary Advance movement. The bounded preflight verifies the three functions and nine deferred trigger names exist and rejects preexisting Collateral repayment, closure, or incompatible Salary Advance exposure evidence. V47 does not change Collateral pricing, offer validity, contract terms, final schedules, Partner state, or post-activation verification evidence.

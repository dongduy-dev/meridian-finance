...
describe('FE-CP9 contract flow', () => {
  it('renders the supported contract-missing waiting state only with contract-phase application context', async () => {
    ...
    expect(await screen.findByText('Your contract is not ready yet')).toBeVisible()
  ...
  it('presents exact versioned terms, masked destination, repayment preview, and operational meaning', async () => {
    ...
    expect(screen.getByText('Contract version 1')).toBeVisible()
  ...
  it('uses the exact displayed version, stays on Contract, and confirms only refreshed current state', async () => {
    ...
    const dialog = await screen.findByRole('dialog', { name: 'Confirm review of contract version 1?' })
    await user.click(within(dialog).getByRole('button', { name: 'Confirm version 1' }))
    expect(await screen.findByText('Contract review confirmed')).toBeVisible()
  ...
  it('refreshes stale version and generates a new identity for deliberate replacement acknowledgment', async () => {
    ...
    renderRoute(`/applications/${applicationId}/contract`, fixture)
    await user.click(await screen.findByRole('button', { name: 'Confirm review of version 1' }))
    await user.click(within(await screen.findByRole('dialog')).getByRole('button', { name: 'Confirm version 1' }))
    expect(await screen.findByText('Review the current contract version')).toBeVisible()
    expect(screen.getByText('Contract version 2')).toBeVisible()
    await user.click(screen.getByRole('button', { name: 'Confirm review of version 2' }))
    await user.click(within(await screen.findByRole('dialog')).getByRole('button', { name: 'Confirm version 2' }))
    await screen.findByText('Contract review confirmed')
  ...
  it('does not mark a newer current version acknowledged from an old-version replay response', async () => {
    ...
    expect(await screen.findByText('Review the current contract version')).toBeVisible()
    expect(screen.getByText('Contract version 2')).toBeVisible()
    expect(screen.queryByText('Contract review confirmed')).not.toBeInTheDocument()
    expect(screen.getByRole('button', { name: 'Confirm review of version 2' })).toBeVisible()
  })
})

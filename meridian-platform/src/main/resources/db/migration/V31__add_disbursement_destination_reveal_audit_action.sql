...
BEGIN
    ...
    IF matching_constraint_count <> 1 THEN
        RAISE EXCEPTION
            'V31 preflight failed: expected V30 audit action constraint is missing or incompatible';
    END IF;
    ...
    IF actual_actions IS NULL
            ...
            OR EXISTS (
                SELECT expected.action FROM unnest(expected_actions) AS expected(action)
                EXCEPT
                SELECT actual.action FROM unnest(actual_actions) AS actual(action)
            )
            OR EXISTS (
                SELECT actual.action FROM unnest(actual_actions) AS actual(action)
                EXCEPT
        ...
        RAISE EXCEPTION
            'V31 preflight failed: audit action constraint does not match expected V30 state';
    END IF;
    ...
    EXECUTE format(
        'CREATE TEMP TABLE v31_expected_audit_action_constraint ('
        'action VARCHAR(100) NOT NULL, '
        'CONSTRAINT chk_v31_expected_audit_action CHECK (action IN (%s))'
        ') ON COMMIT DROP',
        expected_action_sql
    );
    ...
    WHERE relation.relnamespace = pg_my_temp_schema()
      AND relation.relname = 'v31_expected_audit_action_constraint'
      AND constraint_row.conname = 'chk_v31_expected_audit_action'
      AND constraint_row.contype = 'c';
    ...
    IF normalized_constraint_expression IS DISTINCT FROM normalized_expected_expression THEN
        RAISE EXCEPTION
            'V31 preflight failed: incompatible audit action constraint predicate';
    END IF;

    DROP TABLE v31_expected_audit_action_constraint;
END;
$$;

...
BEGIN
    ...
    IF matching_constraint_count <> 1 THEN
        RAISE EXCEPTION
            'V29 preflight failed: expected V28 audit action constraint is missing or incompatible';
    END IF;
    ...
    IF actual_actions IS NULL
        ...
        RAISE EXCEPTION
            'V29 preflight failed: audit action constraint does not match expected V28 state';
    END IF;
    ...
    EXECUTE format(
        'CREATE TEMP TABLE v29_expected_audit_action_constraint ('
        'action VARCHAR(100) NOT NULL, '
        'CONSTRAINT chk_v29_expected_audit_action CHECK (action IN (%s))'
        ') ON COMMIT DROP',
        expected_action_sql
    );
    ...
    WHERE relation.relnamespace = pg_my_temp_schema()
      AND relation.relname = 'v29_expected_audit_action_constraint'
      AND constraint_row.conname = 'chk_v29_expected_audit_action'
      AND constraint_row.contype = 'c';
    ...
    IF normalized_constraint_expression IS DISTINCT FROM normalized_expected_expression THEN
        RAISE EXCEPTION
            'V29 preflight failed: incompatible audit action constraint predicate';
    END IF;

    DROP TABLE v29_expected_audit_action_constraint;
END;
$$;
